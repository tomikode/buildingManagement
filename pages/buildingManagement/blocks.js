...
const BuildingManagement = () => {
	...
	const postBlock = async (newBlock) => {
		...
		try {
			const res = await axios.post("/api/buildingManagement/blocks", blockData);
		...
		<div className={styles.centreWrapper}>
		  ...
		  <div className={styles.contentBox}>
			...
			<div className={styles.titleWithButton}>
				...
				<Button
				  text={
					viewState === VIEW_STATES.BUILDING_MANAGEMENT_LIST ? ADD_BLOCK_LABEL : CANCEL_LABEL
				  }     
				  color={
			...
			{viewState === VIEW_STATES.NEW_BLOCK && <EditBlock onEdit={postBlock}/>}
			{viewState === VIEW_STATES.EDIT_BLOCK && (
			  <EditBlock onEdit={postBlock} block={getBlock(editBlockSelection)}/>
			)}
		  </div>
		</div>
	  </Layout>
